...
class string_parser
{
	...
	bool processContents(unsigned char* buffer, long numRead, LPCSTR filepath, List<String^>^ lst);
public:
	string_parser( STRING_OPTIONS options );
	bool parse_block(unsigned char* buffer, unsigned int buffer_length, LPCSTR datasource, List<String^>^ lst);
	bool parse_stream(FILE* fh, LPCSTR datasource, List<String^>^ lst);
	~string_parser(void);
};