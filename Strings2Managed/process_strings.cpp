...
bool process_strings::dump_system(List<String^>^ lst)
{
	...
	if( hSnapShot != INVALID_HANDLE_VALUE )
	{
		...
		{
			if( result == TRUE )
				dump_process(tmpProcess.th32ProcessID, lst);
...
bool process_strings::dump_process(DWORD pid, List<String^>^ lst)
{
	// Open the process
	...
	if( ph != NULL )
	{
		...
		if ( hSnapshot != INVALID_HANDLE_VALUE )
		{
			...
			bool result = this->processAllHeaps(ph, process_name, lst);
...
bool process_strings::processAllHeaps(HANDLE ph, char* process_name, List<String^>^ lst)
{
		...
		if( mbi.State == MEM_COMMIT && !(mbi.Protect & (PAGE_NOACCESS | PAGE_GUARD)) )
		{
			...
			if( buffer != NULL )
			{
				...
				if( numRead > 0 )
				{
					...
					parser->parse_block( buffer, numRead, process_name, lst);
				}else if( !result ){
